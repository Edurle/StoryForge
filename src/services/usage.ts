...
export async function getUsageSummary(
  ...
  const res = await w.request({
    id: 0,
    type: "query",
    sql: "SELECT COUNT(*) as totalCalls, COALESCE(SUM(prompt_tokens), 0) as totalPromptTokens, COALESCE(SUM(completion_tokens), 0) as totalCompletionTokens, COALESCE(SUM(cache_hit_tokens), 0) as totalCacheHitTokens FROM api_usage",
  });
  if (!res.ok || !res.data) return { totalCalls: 0, totalPromptTokens: 0, totalCompletionTokens: 0, totalCacheHitTokens: 0 };
  const row = (res.data as Array<Record<string, number>>)[0];
  if (!row) return { totalCalls: 0, totalPromptTokens: 0, totalCompletionTokens: 0, totalCacheHitTokens: 0 };
  return {
